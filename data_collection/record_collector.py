...
def process_cve(cve):
    ...
    try:
        ...
        if already_exists or severity == "N/A":
            print(f"CVE {cve_data['cve_id']} already exists in the database or has no severity. Skipping.")
        else:
            add_cve(cve_data)
            added = True
    except Exception as e:
        print(f"Error checking existence of CVE: {e}")
    return cve_data, added
